...
class Battery(object):
    ...
    def manage_charge_voltage(self):
        if LIMITATION_MODE == "Linear":
            return self.manage_charge_voltage_linear() # return self.manage_charge_voltage_step()
 ...
 #  def manage_discharge_current(self):
    ...
    def calcMaxChargeCurrentReferringToSoc(self):
        try:
            ...
            #                              98                95           80            14               8
            SOC_WHILE_CHARGING = [100, CC_SOC_LIMIT1, CC_SOC_LIMIT2, CC_SOC_LIMIT3, CC_SOC_LIMIT4, CC_SOC_LIMIT5, 0]
            MAX_CHARGE_CURRENT_SOC = [5, CC_CURRENT_LIMIT1, CC_CURRENT_LIMIT2, CC_CURRENT_LIMIT3, MAX_BATTERY_CHARGE_CURRENT, CC_CURRENT_LIMIT5, 5]
            if LIMITATION_MODE == "Linear":
    ...
    def calcMaxDischargeCurrentReferringToSoc(self):
        try:
            # Create value list. Will more this to the settings object
            SOC_WHILE_DISCHARGING = [100, DC_SOC_LIMIT3, DC_SOC_LIMIT2, DC_SOC_LIMIT1]
            MAX_DISCHARGE_CURRENT_SOC = [MAX_BATTERY_DISCHARGE_CURRENT,DC_CURRENT_LIMIT3,DC_CURRENT_LIMIT2, DC_CURRENT_LIMIT1]
            if LIMITATION_MODE == "Linear":
                return calcLinearRelationship(self.soc, SOC_WHILE_DISCHARGING, MAX_DISCHARGE_CURRENT_SOC)
            return calcStepRelationship(self.soc, SOC_WHILE_DISCHARGING, MAX_DISCHARGE_CURRENT_SOC, True)
        except:
            return self.max_battery_discharge_current
    ...
    def get_max_cell(self):
        max_voltage = 0
#        correctionvoltage = CellConectionResistance*self.current              self.battery.current
        max_cell = None
        ...
        for c in range(min(len(self.cells), self.cell_count)):
            if self.cells[c].voltage is not None and max_voltage < self.cells[c].voltage:
                 ...
                 max_cell = c
#               max_voltage = max_voltage- correctionvoltage
        return max_cell
    ...
    def get_min_cell_voltage(self):
        min_voltage = None
        correctionvoltage = CellConectionResistance*self.current
        ...
        if min_voltage is None:
            try:
                ...
                pass
        min_voltage = min_voltage - correctionvoltage
        return min_voltage
